...
- 商品详情页增强内容：当前缺少“加入项目”“导出数据”等右侧快捷操作。
- 公司库页面增强内容：当前缺少公司类型 / 主营分类筛选和平台数据区。
...
- 支持上传并关联商品、公司或报价
- 支持维护 `document_kind`、`is_primary`、`sort_order`、`source_sheet_name`、`series_id`、`variant_id`、`quote_batch_id`
...
- 管理员可维护报价主体、关联商品、关联公司、币种、批次发布时间、生效日期、状态、备注
- 支持多阶梯价录入与批次级资料附件绑定
- 支持报价附件或报价单关联
- 前台详情页可读取报价记录摘要
...
- `[x]` 独立动态页改为读取真实 `updates` / `products` / `quotes` / `documents` 组合数据
- `[x]` 商品详情补齐“相关动态”区块
- `[x]` 首页搜索支持跳转到产品、资料、报价、公司等不同目标页
- `[x]` 公司库补齐关联产品数、报价数和动态痕迹
...
建议下一轮优先围绕“权限回归”和“真实 Cloud 演示数据导入验收”推进：

1. `[ ]` 执行权限回归验证：匿名不可读写、普通用户只读、管理员可写。
2. `[ ]` 在真实 Supabase Cloud 执行 `pnpm demo:seed`、`pnpm demo:assets`、`pnpm demo:upload`，确认 32 份资料全部入 bucket。
3. `[ ]` 用真实测试账号完成前后台联调验收，补齐 6 个重点变体串讲截图。

## 迭代 4：演示数据环境收口

### 4.1 演示数据真源

- `[x]` 新增 `demo-data/catalog.json` 作为分类、品牌、公司、系列、变体、资料、报价和动态的单一真源
- `[x]` 新增 `scripts/demo-data/generate-seed.mjs`，从真源生成 `supabase/seed.sql`
- `[x]` 新增 `scripts/demo-data/generate-assets.py` 和 `scripts/demo-data/upload-assets.mjs`

验收标准：

- `supabase/seed.sql` 可从 `demo-data/catalog.json` 重生成
- 演示数据计数固定覆盖 4 分类、8 品牌、6 公司、12 系列、24 变体、32 份资料、10 条报价线、12 条动态
- 资料对象写入 `document_kind`、`series_id`、`variant_id`、`quote_batch_id`、`is_primary`

### 4.2 前后台演示链路

- `[x]` 首页、动态页、详情页、资料页、报价页、公司库统一消费同一套真实 API / fallback 真源
- `[x]` 后台动态、资料、报价管理页补齐批次、关联对象和结构化字段
- `[x]` 报价链路统一输出批次标题和阶梯价摘要
